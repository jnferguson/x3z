...
hexdump_t::append(const uint8_t* ptr, const std::size_t siz)
{
	if ( nullptr == ptr || 0 == siz )
		throw std::exception("hexdump_t::append(): Invalid parameter(s)");
...
{
	std::string 		brk(""); // ' ', m_break);
	unsigned char		unp(m_unprint);
	std::string			retval("");

	for ( std::size_t idx = 0; idx < m_break; idx++ )
		brk += ' ';
	if ( 0 == m_break )
		brk = ' ';
	...
	if ( cnt > m_data.size() )
		throw std::exception("hexdump_t::get_line(): Invalid index specified");
...
val = get_line(idx).c_str();
emit update(QString(val + "\r\n"));
}
val = get_line(idx).c_str();
emit update(val);
} else {
val = get_line(0).c_str();
emit update(val);
}
